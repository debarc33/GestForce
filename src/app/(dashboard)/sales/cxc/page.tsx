...
function filterByAging(rows: CarteraRow[], filter: string): CarteraRow[] {
...
export default function CxCPage() {
  ...
  const { data: cartera = [], isLoading } = useCartera(cid)
  ...
  const totalCartera  = cartera.reduce((s, r) => s + r.balance_due, 0)
  const overdueCount  = cartera.filter(r => r.dias_vencido > 0).length
  ...
  const onExport = () => {
    ...
    exportToExcel(filtered, cols, `cxc_${new Date().toISOString().split('T')[0]}`)
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-zinc-900 tracking-tight">Cuentas por Cobrar</h1>
        <p className="mt-0.5 text-[13px] text-zinc-400">Facturas pendientes de cobro a clientes.</p>
      </div>

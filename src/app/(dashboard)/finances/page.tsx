...
export default function FinancesPage() {
  ...
  const PeriodSelector = () => (
    <div className="flex items-center gap-2 text-sm">
      ...
      <input
        ...
        className="rounded-lg border border-[var(--glass-border)] bg-[var(--glass)] px-2 py-1.5 text-sm text-foreground focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/15"
      />
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-foreground tracking-tight">Finanzas</h1>
        <p className="mt-0.5 text-[13px] text-muted-foreground">Resumen financiero y fiscal de tu empresa.</p>
      </div>
        ...
        {TABS.map(tab => (
          <button key={tab.id} type="button"
            ...
            className={`relative px-4 py-2.5 text-sm font-medium transition-colors ${
              activeTab === tab.id ? 'text-[var(--accent)]' : 'text-muted-foreground hover:text-foreground'
            }`}>
            {tab.label}
            {activeTab === tab.id && (
        ...
        <div className="space-y-6">
          ...
          {loadingPanel ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {[1,2,3,4,5].map(i => <div key={i} className="h-28 animate-pulse rounded-xl bg-[var(--glass-hover)]" />)}
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {/* Ingresos */}
              <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass)] p-4 shadow-sm col-span-1">
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-8 w-8 rounded-lg bg-[var(--accent-bg)] flex items-center justify-center">
                    <TrendingUp className="h-4 w-4 text-[var(--accent)]" />
                  </div>
                  <p className="text-xs font-medium text-muted-foreground">Ingresos</p>
                </div>
                <p className="text-xl font-bold text-foreground tabular-nums">{fmtCOP(panel?.ingresos ?? 0)}</p>
                <p className="text-xs text-muted-foreground mt-0.5">Facturas emitidas en el período</p>
              </div>

              {/* Gastos */}
              <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass)] p-4 shadow-sm col-span-1">
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-8 w-8 rounded-lg bg-[var(--glass-hover)] flex items-center justify-center">
                    <TrendingDown className="h-4 w-4 text-[var(--accent)]" />
                  </div>
                  <p className="text-xs font-medium text-muted-foreground">Gastos</p>
                </div>
                <p className="text-xl font-bold text-foreground tabular-nums">{fmtCOP(panel?.gastos ?? 0)}</p>
                <p className="text-xs text-muted-foreground mt-0.5">Facturas de proveedor en el período</p>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className={`rounded-xl border border-[var(--glass-border)] p-4 shadow-sm col-span-1 bg-[var(--glass)]`}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className={`h-8 w-8 rounded-lg flex items-center justify-center bg-[var(--glass-hover)]`}>
                        <Scale className={`h-4 w-4 text-[var(--accent)]`} />
                      </div>
                      <p className="text-xs font-medium text-muted-foreground">IVA neto</p>
                    </div>
                    <p className={`text-xl font-bold tabular-nums text-foreground`}>{fmtCOP(Math.abs(iva))}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{positive ? 'A pagar a la DIAN' : 'Saldo a favor'}</p>
                  </div>
                )
              })()}
              ...
              <div className={`rounded-xl border border-[var(--glass-border)] p-4 shadow-sm col-span-1 bg-[var(--glass)]`}>
                ...
                <p className="text-xl font-bold text-foreground tabular-nums">{fmtCOP(panel?.cartera ?? 0)}</p>
                <Link href="/sales?tab=cxc" className="text-xs text-[var(--accent)] hover:underline mt-0.5 block">
                  Ver detalle CxC →
                </Link>
              </div>
              ...
              <div className={`rounded-xl border border-[var(--glass-border)] p-4 shadow-sm col-span-1 bg-[var(--glass)]`}>
                ...
                <p className="text-xl font-bold text-foreground tabular-nums">{fmtCOP(panel?.obligaciones ?? 0)}</p>
                <Link href="/purchases?tab=cxp" className="text-xs text-[var(--accent)] hover:underline mt-0.5 block">
                  Ver detalle CxP →
                </Link>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass)] p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-foreground">Ingresos vs. Gastos — {year}</h3>
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-sm bg-[var(--accent)]" />Ingresos</span>
                <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-sm bg-[var(--glass-hover)]" />Gastos</span>
              </div>
            </div>
            {(() => {
                    ...
                    {bars.map(bar => (
                      <div key={bar.mes} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                        <div className="flex items-end gap-0.5 w-full justify-center" style={{ height: '128px' }}>
                          <div
                            className="w-4 bg-[var(--accent)] rounded-t transition-all duration-500"
                            style={{ height: `${Math.max(2, (bar.ingresos / maxVal) * 128)}px` }}
                            title={`Ingresos ${bar.mes}: ${fmtCOP(bar.ingresos)}`}
                          />
                          <div
                            className="w-4 bg-[var(--glass-hover)] rounded-t transition-all duration-500"
                            style={{ height: `${Math.max(2, (bar.gastos / maxVal) * 128)}px` }}
                            title={`Gastos ${bar.mes}: ${fmtCOP(bar.gastos)}`}
                          />
                        </div>
                        <span className="text-[10px] text-muted-foreground">{bar.mes}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <button
              ...
              disabled={!tax}
              className="flex items-center gap-1.5 rounded-lg border border-[var(--glass-border)] bg-[var(--glass)] px-3 py-2 text-sm font-medium text-foreground hover:bg-[var(--glass-hover)] disabled:opacity-40 transition-colors">
          ...
          {loadingTax ? (
            <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-36 animate-pulse rounded-xl bg-[var(--glass-hover)]" />)}</div>
          ) : tax ? (
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="border-t border-[var(--glass-border)] pt-2 mt-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-foreground">Saldo neto a declarar</span>
                      <div className="flex items-center gap-2">
                        {tax.ivaNeto < 0 && (
                          <span className="rounded-full bg-[var(--glass-hover)] px-2 py-0.5 text-xs font-medium text-foreground">
                            Saldo a favor
                          </span>
                        )}
              ...
              <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass)] p-5 shadow-sm">
                ...
                {tax.reteRecibida === 0 && tax.reteIvaRecibida === 0 && (
                  <div className="mt-3 flex items-start gap-2 rounded-lg bg-[var(--glass-hover)] border border-[var(--glass-border)] px-3 py-2.5">
              ...
              <div className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass)] p-5 shadow-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between py-1">
                    <span className="text-sm text-muted-foreground">Tasa ICA configurada</span>
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  ...
                  <Link href="/settings" className="text-[var(--accent)] hover:underline">Configuración → Empresa</Link>.
                </p>
              </div>
...
function TaxLine({ label, value, bold }: { label: string; value: number; bold?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span className={`text-sm tabular-nums ${bold ? 'font-bold text-foreground' : 'font-medium text-foreground'} ${neg ? 'text-red-600' : ''}`}>

...
import { useSuppliers } from '@/modules/suppliers/queries'
import { useCompanyStore } from '@/store/useCompanyStore'
import { createPurchaseOrderWithItems } from '@/modules/purchases/queries'
import type { PurchaseItemValues } from '@/modules/purchases/schemas'
...
function recalc(item: PurchaseItemValues): PurchaseItemValues {
...
export default function NewPurchaseOrderPage() {
  ...
  const { data: suppliers = [] } = useSuppliers(activeCompanyId ?? undefined)
  ...
  const updateField = (index: number, field: keyof PurchaseItemValues, value: number) => {
    setItems(prev => {
      const updated = [...prev]
    ...
    mutationFn: () =>
      createPurchaseOrderWithItems({
        ...
        items: items.map(({ product_id, product_name, sku, quantity, unit_price, discount, tax_rate, subtotal, tax, total }) => ({
          product_id, product_name, sku: sku ?? null, quantity, unit_price, discount, tax_rate, subtotal, tax, total,
        })),
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchase_orders', activeCompanyId] })
    ...
    <div className="max-w-5xl mx-auto space-y-4 pb-10">
      ...
      <div className="flex items-center justify-between py-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-zinc-900">Nueva orden de compra</h1>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-3">
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="col-span-2 relative">
            ...
            {supplierId && selectedSupplier ? (
              <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2">
                <span className="flex-1 text-sm text-zinc-900 truncate">{selectedSupplier.name}</span>
                <button onClick={() => { setSupplierId(''); setSupplierSearch('') }}
                    ...
                    {supplierResults.map(s => (
                      <button key={s.id}
                        ...
                        <div>
                          ...
                          {s.doc_number && <p className="text-xs text-zinc-400 font-mono">{s.doc_type} {s.doc_number}</p>}
                        </div>
                      </button>
                    ))}
                  </div>
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-500 mb-1">Entrega esperada</label>
      ...
      <div className="grid grid-cols-[1fr_300px] gap-4 items-start">
        <div>
          <label className="block text-xs font-medium text-zinc-500 mb-1.5">Notas / condiciones</label>
          <textarea value={notes} onChange={e => setNotes(e.target.value)}
            rows={4} placeholder="Condiciones de entrega, observaciones..."
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-4 space-y-2">
          ...
          <div className="flex justify-between text-sm text-zinc-600">
            <span>IVA (descontable)</span><span>{fmt(totals.tax)}</span>
          </div>
